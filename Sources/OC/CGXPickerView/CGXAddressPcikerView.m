...
+ (void)showAddressPickerWithTitle:(NSString *)title
                   DefaultSelected:(NSArray *)defaultSelectedArr
                      ...
                      IsAutoSelect:(BOOL)isAutoSelect
                       ...
                       ResultBlock:(CGXAddressResultBlock)resultBlock
{
    CGXAddressPcikerView *addressPickerView = [[CGXAddressPcikerView alloc] initWithTitle:title DefaultSelected:defaultSelectedArr dataSource:dataSource IsAutoSelect:isAutoSelect Manager:manager ResultBlock:resultBlock];
    [addressPickerView showWithAnimation:YES];
}
...

{
    if (self = [super init]) {
        ...
        [self getData:[dataSource mutableCopy]];
        [self initUI];
    }
    return self;
}
...
- (void)loadData {
    ...
    if (self.fileName) {
        ...
        NSString *strResourcesBundle = [[NSBundle mainBundle] pathForResource:@"WLYUIKitBaseOC_WLYUIKitBaseOC" ofType:@"bundle"];
        NSBundle * bundle = [NSBundle bundleWithPath:strResourcesBundle];
        NSString *filePath = [bundle pathForResource:@"CGXAddressCity" ofType:@"plist"];
        arrData = [NSMutableArray arrayWithContentsOfFile:filePath];
    }
    [self getData:arrData];
}
...
- (void)clickRightBtn {
    ...
    if(self.resultBlock) {
        ...
        [rowAry addObject:[NSString stringWithFormat:@"%ld",(long)rowOfProvince]];
        [rowAry addObject:[NSString stringWithFormat:@"%ld",(long)rowOfCity]];
        [rowAry addObject:[NSString stringWithFormat:@"%ld",(long)rowOfTown]];
    ...
    if (rowOfProvince < self.addressModelArr.count) {
        CGXProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
            if (rowOfCity < provinceModel.city.count) {
                CGXCityModel *cityModel = provinceModel.city[rowOfCity];
                arr = @[provinceModel.name, cityModel.name, @"--"];
