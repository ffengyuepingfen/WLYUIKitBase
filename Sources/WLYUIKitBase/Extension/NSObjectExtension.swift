...
public extension NSObject {
    ...
    static var w_identifier: String {
        return String(describing: self) + "w_identifier"
    }
}
#endif
...
@objc public extension NSObject {
    ...
    static func hookInstanceMethod(of origSel: Selector, with replSel: Selector) -> Bool {
        ...
        guard let oriMethod = class_getInstanceMethod(clz, origSel) as Method? else {
            GConfig.log("原 实例方法：Swizzling Method(\(origSel)) not found while swizzling class \(NSStringFromClass(classForCoder())).")
            return false
        }
        
        guard let repMethod = class_getInstanceMethod(clz, replSel) as Method? else {
            GConfig.log("新 实例方法：Swizzling Method(\(replSel)) not found while swizzling class \(NSStringFromClass(classForCoder())).")
            return false
        }
    ...
    static func hookClassMethod(of origSel: Selector, with replSel: Selector) -> Bool {
        ...
        guard let oriMethod = class_getClassMethod(clz, origSel) as Method? else {
            GConfig.log("原 类方法：Swizzling Method(\(origSel)) not found while swizzling class \(NSStringFromClass(classForCoder())).")
            return false
        }
        
        guard let repMethod = class_getClassMethod(clz, replSel) as Method? else {
            GConfig.log("新 类方法 replSel：Swizzling Method(\(replSel)) not found while swizzling class \(NSStringFromClass(classForCoder())).")
            return false
        }
    ...
    static func hookMethod(of origSel: Selector, with replSel: Selector, isClassMethod: Bool) -> Bool {
        ...
        guard let oriMethod = (isClassMethod ? class_getClassMethod(clz, origSel) : class_getClassMethod(clz, origSel)) as Method?,
            ...
            GConfig.log("Swizzling Method(s) not found while swizzling class \(NSStringFromClass(classForCoder())).")
            return false
        }
...
@objc public extension NSObject{
    ...
    private func hook_setValue(_ value: Any?, forUndefinedKey key: String) {
        GConfig.log("setValue: forUndefinedKey:, 未知键Key: \(key)")
    }
    
    private func hook_value(forUndefinedKey key: String) -> Any? {
        GConfig.log("valueForUndefinedKey:, 未知键: \(key)")
        return nil
    }
    
    private func hook_setNilValueForKey(_ key: String) {
        GConfig.log("Invoke setNilValueForKey:, 不能给非指针对象(如NSInteger)赋值 nil")
