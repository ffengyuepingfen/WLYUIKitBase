...
//        let dateFormatter = DateFormatter.cicDefault(format: .yyyyMMdd)
...
//    }
    ...
    //日期 -> 字符串
    public static func date2String(_ date: Date, dateFormat: DateFormatterOptions = .yyyyMMddHHmmss) -> String {
        let formatter = DateFormatter.cicDefault(format: dateFormat)
        let date = formatter.string(from: date)
        return date
    }
    
    //字符串 -> 日期
    public static func string2Date(_ string: String, local: String = "", dateFormat: DateFormatterOptions = .yyyyMMddHHmmss) -> Date? {
        let formatter = DateFormatter.cicDefault(format: dateFormat)
        if local != "" {
    ...
    /// 当前时间
    public static func nowString(dateFormat: DateFormatterOptions = .yyyyMMddHHmmss) -> String {
        let formatter = DateFormatter.cicDefault(format: dateFormat)
...
extension Date {
    ...
    public static func dateTodate(date1:String,date2:String,format:DateFormatterOptions = .yyyyMMdd) -> String {
        let formatter = DateFormatter.cicDefault(format: format)
    ...
    public static func getDateYearMonths(n: NSInteger) -> [String] {
        ...
        //设定数据格式为xxxx-mm
        let formatter = DateFormatter.cicDefault(format: .yyyyMM)
