...
public extension UIImage {
    ...
    static func textImage(_ text: String, fontSize: CGFloat = 16, size: (CGFloat, CGFloat), backgroundColor: UIColor = UIColor.orange, textColor: UIColor = UIColor.white, isCircle: Bool = true, isFirstChar: Bool = false) -> UIImage? {
        ...
        let textsize = text.rectSize(font: UIFont.systemFont(ofSize: fontSize), size: CGSize(width: GConfig.ScreenW, height: CGFloat(MAXFLOAT)))
    ...
    func pixelColor(_ point: CGPoint) -> UIColor? {
        ...
        let pixelData = (self.size.width * point.y + point.x) * numberOfComponents
...
public extension UIImage {
    ...
    func scaleTo(size: CGSize) -> UIImage? {
        ...
        UIGraphicsBeginImageContext(size)
        self.draw(in: CGRect(x: xPos, y: yPos, width: w, height: h))
    ...
    func strechBubble(edgeInsets: UIEdgeInsets, resizingMode: UIImage.ResizingMode = .stretch) -> UIImage {
        // 拉伸
        return self.resizableImage(withCapInsets: edgeInsets, resizingMode: resizingMode)
    }
...
public extension UIImage {
    ...
    func compress(mode: CompressionMode = .medium) -> Data? {
        return resizeIO(resizeSize: mode.resize(self.size))?.compressDataSize(maxSize: mode.maxDataSize)
    }
    ...
    private func cycleCompressDataSize(maxSize: Int) -> Data? {
        guard let oldData = self.jpegData(compressionQuality: 1) else { return nil }
...
public extension UIImage {
    ...
    static func gif(data: Data) -> UIImage? {
        ...
        guard let source = CGImageSourceCreateWithData(data as CFData, nil) else {
            GConfig.log("SwiftGif: Source for the image does not exist")
            return nil
        }
    ...
    static func gif(url: String) -> UIImage? {
        ...
        guard let bundleURL = URL(string: url) else {
            GConfig.log("SwiftGif: This image named \"\(url)\" does not exist")
            return nil
        }
        // Validate data
        guard let imageData = try? Data(contentsOf: bundleURL) else {
            GConfig.log("SwiftGif: Cannot turn image named \"\(url)\" into NSData")
            return nil
        }
    ...
    static func gif(name: String) -> UIImage? {
        ...
        guard let bundleURL = Bundle.main
            ...
            GConfig.log("SwiftGif: This image named \"\(name)\" does not exist")
                    return nil
                }
        // Validate data
        guard let imageData = try? Data(contentsOf: bundleURL) else {
            GConfig.log("SwiftGif: Cannot turn image named \"\(name)\" into NSData")
            return nil
        }
    ...
    static func gif(asset: String) -> UIImage? {
        ...
        guard let dataAsset = NSDataAsset(name: asset) else {
            GConfig.log("SwiftGif: Cannot turn image named \"\(asset)\" into NSDataAsset")
            return nil
        }
