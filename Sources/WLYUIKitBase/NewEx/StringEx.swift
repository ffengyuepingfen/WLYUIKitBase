...
extension String {
    ...
    public func transformFormate() -> String {
        ...
        //格式话输出
        let dformatter = DateFormatter.cicDefault(format: .HHmmss)
    ...
    public func getDayOfWeek() -> String {
        let formatter  = DateFormatter.cicDefault(format: .yyyyMMdd)
...
public extension ExpressibleByStringLiteral {
    ...
    func toClass() -> AnyClass? {
        // 1.动态获取命名空间
        let namespace = Bundle.infoDic!["CFBundleExecutable"] as! String
        // 2.将字符串转换为类
        // 2.1.默认情况下命名空间就是项目的名称，但是命名空间的名称是可以更改的
        guard let Class: AnyClass = NSClassFromString(namespace.removeSomeStringUseSomeString(removeString: " ", replacingString: "_") + "." + (self as! String)) else {
            return nil
        }
        return Class
    }
    ...
    func toPinyin(_ isTone: Bool = false) -> String {
        let mutableString = NSMutableString(string: self as! String)
    ...
    func separatedByString(with char: String) -> Array<Any> {
        return (self as! String).components(separatedBy: char)
    }
    ...
    func getUrls() -> [String]? {
        ...
        // 匹配字符串，返回结果集
        let res = dataDetector.matches(in: self as! String, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, (self as! String).count))
        // 取出结果
    ...
    func setHtmlAttributedString(font: UIFont? = UIFont.systemFont(ofSize: 16), lineSpacing: CGFloat? = 10) -> NSMutableAttributedString {
        ...
        do {
            if let data = (self as! String).replacingOccurrences(of: "\n", with: "<br/>").data(using: .utf8) {
        ...
        return htmlString ?? NSMutableAttributedString(string: self as! String)
    }
    ...
    func countOfChars() -> Int {
        var count = 0
        guard (self as! String).count > 0 else { return 0 }
        for i in 0...(self as! String).count - 1 {
...
public extension ExpressibleByStringLiteral {
    ...
    var removeBeginEndAllSapcefeed: String {
        let resultString = (self as! String).trimmingCharacters(in: CharacterSet.whitespaces)
        return resultString
    }
    ...
    var removeBeginEndAllLinefeed: String {
        let resultString = (self as! String).trimmingCharacters(in: CharacterSet.newlines)
        return resultString
    }
    ...
    var removeBeginEndAllSapceAndLinefeed: String {
        var resultString = (self as! String).trimmingCharacters(in: CharacterSet.whitespaces)
    ...
    var removeAllSapce: String {
        return (self as! String).replacingOccurrences(of: " ", with: "", options: .literal, range: nil)
    }
    
    // MARK: 3.5、去掉所有换行
    /// 去掉所有换行
    var removeAllLinefeed: String {
        return (self as! String).replacingOccurrences(of: "\n", with: "", options: .literal, range: nil)
    }
    ...
    var removeAllLineAndSapcefeed: String {
        // 去除所有的空格
        var resultString = (self as! String).replacingOccurrences(of: " ", with: "", options: .literal, range: nil)
        // 去除所有的换行
    ...
    func isValidNumber() -> Bool {
        /// 0-9的数字，也不包含小数点
        let rst: String = (self as! String).trimmingCharacters(in: .decimalDigits)
        if rst.count > 0 {
            return false
        }
        return true
    ...
    func urlEncode() -> String {
        ...
        var charSet = CharacterSet.urlQueryAllowed
        charSet.insert(charactersIn: "%")
        return (self as! String).addingPercentEncoding(withAllowedCharacters: charSet) ?? ""
    }
    
    // MARK: 3.9、url进行解码
    /// url解码
    func urlDecode() -> String {
        return (self as! String).removingPercentEncoding ?? ""
    }
    ...
    func removeSomeStringUseSomeString(removeString: String, replacingString: String = "") -> String {
        return (self as! String).replacingOccurrences(of: removeString, with: replacingString)
    }
    ...
    func pregReplace(pattern: String, with: String,
        ...
        return regex.stringByReplacingMatches(in: (self as! String), options: [],
                                              range: NSMakeRange(0, (self as! String).count),
                                              withTemplate: with)
    }
    ...
    func removeCharacter(characterString: String) -> String {
        ...
        return (self as! String).trimmingCharacters(in: characterSet)
    }
}
...
public extension ExpressibleByStringLiteral {
    ...
    /// - Returns: Bool
    func toBool() -> Bool? {
        let trimmedString = (self as! String).lowercased()
    ...
    var toNSString: NSString {
        return (self as! String) as NSString
    }
    
    // MARK: 4.8、字符串转 Int64
    /// 字符串转 Int64
    var toInt64Value: Int64 {
        return Int64(self as! String) ?? 0
    }
...
extension ExpressibleByStringLiteral {
    ...
    public func rectSize(font: UIFont, size: CGSize) -> CGSize {
        ...
        let rect: CGRect = (self as! String).boundingRect(with: size, options: option, attributes: attributes, context: nil)
        return rect.size
    }
    ...
    public func singleLineSize(font: UIFont) -> CGSize {
        ...
        return (self as! String).size(withAttributes: attrs as [NSAttributedString.Key: Any])
    }
    ...
    public func singleLineWidth(font: UIFont) -> CGFloat {
        let attrs = [NSAttributedString.Key.font: font]
        return (self as! String).size(withAttributes: attrs as [NSAttributedString.Key: Any]).width
    }
    ...
    public func singleLineHeight(font: UIFont) -> CGFloat {
        let attrs = [NSAttributedString.Key.font: font]
        return (self as! String).size(withAttributes: attrs as [NSAttributedString.Key: Any]).height
    }
...
//        let label = UILabel(frame: rect).font(font).text((self as! String)).line(0)
...
//        let label = UILabel(frame: rect).font(font).text((self as! String)).line(0)
//        let attrStr = NSMutableAttributedString(string: (self as! String))
//        let paragraphStyle = NSMutableParagraphStyle()
//        paragraphStyle.lineSpacing = lineSpacing
//        attrStr.addAttribute(NSAttributedString.Key.paragraphStyle, value: paragraphStyle, range: NSMakeRange(0, (self as! String).count))
...
public extension ExpressibleByStringLiteral {
    ...
    func toThousands() -> String? {
        ...
        var num = NSDecimalNumber(string: (self as! String))
    ...
    func accuraterDouble() -> Double? {
        guard let decimal = Decimal(string: (self as! String)) else { return nil }
        GConfig.log(NSDecimalNumber(decimal: decimal).doubleValue)
    ...
    func cutLastZeroAfterDot() -> String {
        var rst = (self as! String)
        var i = 1
        if (self as! String).contains(".") {
            while i < (self as! String).count {
                if rst.hasSuffix("0") {
                    rst.removeLast()
                    i = i + 1
                } else {
    ...
    func saveNumberDecimal(numberDecimal: Int = 0, mode: NumberFormatter.RoundingMode = .floor) -> String {
        var n = NSDecimalNumber(string: (self as! String))
...
extension ExpressibleByStringLiteral {
    ...
    public var isValidIDCardNumStrict: Bool {
        let str = (self as! String).trimmingCharacters(in: NSCharacterSet.whitespacesAndNewlines)
        let len = str.count
    ...
    public func stringWithHighLightSubstring(keyword: String, normalColor: UIColor, keywordCololor: UIColor, isSplit: Bool = false, options: NSRegularExpression.Options = []) -> NSMutableAttributedString {
        let totalString = (self as! String)
    ...
    public var isVideoUrl: Bool {
        if (self as! String).hasSuffix("mp4") || (self as! String).hasSuffix("MP4") || (self as! String).hasSuffix("MOV") || (self as! String).hasSuffix("mov") || (self as! String).hasSuffix("mpg") || (self as! String).hasSuffix("mpeg") || (self as! String).hasSuffix("mpg4") || (self as! String).hasSuffix("wm") || (self as! String).hasSuffix("wmx") || (self as! String).hasSuffix("mkv") || (self as! String).hasSuffix("mkv2") || (self as! String).hasSuffix("3gp") || (self as! String).hasSuffix("3gpp") || (self as! String).hasSuffix("wv") || (self as! String).hasSuffix("wvx") || (self as! String).hasSuffix("avi") || (self as! String).hasSuffix("asf") || (self as! String).hasSuffix("fiv") || (self as! String).hasSuffix("swf") || (self as! String).hasSuffix("flv") || (self as! String).hasSuffix("f4v") || (self as! String).hasSuffix("m4u") || (self as! String).hasSuffix("m4v") || (self as! String).hasSuffix("mov") || (self as! String).hasSuffix("movie") || (self as! String).hasSuffix("pvx") || (self as! String).hasSuffix("qt") || (self as! String).hasSuffix("rv") || (self as! String).hasSuffix("vod") || (self as! String).hasSuffix("rm") || (self as! String).hasSuffix("ram") || (self as! String).hasSuffix("rmvb") {
            return true
        }
        return false
    }
...
extension ExpressibleByStringLiteral {
    ...
    public func sub(to index: Int) -> String {
        ...
        return String((self as! String)[(self as! String).startIndex ..< end_Index])
    }
    ...
    public func sub(from index: Int) -> String {
        ...
        return String((self as! String)[start_index ..< (self as! String).endIndex])
    }
    ...
    public func sub(start: Int, length: Int = -1) -> String {
        var len = length
        if len == -1 {
            len = (self as! String).count - start
        }
        let st = (self as! String).index((self as! String).startIndex, offsetBy: start)
        let en = (self as! String).index(st, offsetBy: len)
        let range = st ..< en
        return String((self as! String)[range]) // .substring(with:range)
    }
    ...
    public func slice(_ range: CountableRange<Int>) -> String {
        ...
        return String((self as! String)[startIndex ..< endIndex])
    }
    ...
    private func position(of sub: String, backwards: Bool = false) -> Int {
        var pos = -1
        if let range = (self as! String).range(of: sub, options: backwards ? .backwards : .literal) {
            if !range.isEmpty {
                pos = (self as! String).distance(from: (self as! String).startIndex, to: range.lowerBound)
            }
        }
        return pos
    }
    ...
    public func range(of subString: String) -> Range<String.Index>? {
        return (self as! String).range(of: subString)
    }
    ...
    public func insertString(content: String, locat: Int) -> String {
        guard locat < (self as! String).count else {
            return (self as! String)
        }
        let str1 = (self as! String).sub(to: locat)
        let str2 =  (self as! String).sub(from: locat + 1)
...
extension ExpressibleByStringLiteral {
    ...
    public func urlEncoding(characters: CharacterSet = .urlQueryAllowed) -> String {
        let encodeUrlString = (self as! String).addingPercentEncoding(withAllowedCharacters:
                                                                        characters)
...
public extension ExpressibleByStringLiteral {
    ...
    func decimalToBinary() -> String {
        guard var decimal = (self.self as! String).toInt() else {
            return ""
        }
        var str = ""
        while decimal > 0 {
    ...
    func decimalToOctal() -> String {
        guard let decimal = (self.self as! String).toInt() else {
            return ""
        }
        /*
...
public extension ExpressibleByStringLiteral {
    ...
    func color(_ color: UIColor) -> NSMutableAttributedString {
        let attributedText = NSMutableAttributedString(string: self.self as! String, attributes: [.foregroundColor: color])
    ...
    func font(_ font: CGFloat) -> NSMutableAttributedString {
        let attributedText = NSMutableAttributedString(string: self.self as! String, attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: font)])
        return attributedText
    }
    ...
    func font(_ font: UIFont) -> NSMutableAttributedString {
        let attributedText = NSMutableAttributedString(string: self.self as! String, attributes: [NSAttributedString.Key.font: font])
        return attributedText
    }
    ...
    func text() -> NSMutableAttributedString {
        let attributedText = NSMutableAttributedString(string: self.self as! String)
        return attributedText
    }
    ...
    func strikethrough() -> NSMutableAttributedString {
        let attributedText = NSMutableAttributedString(string: self.self as! String, attributes: [.strikethroughStyle: NSUnderlineStyle.single.rawValue])
        return attributedText
    }
}

// MARK: - 十四、MD5 加密 和 self64 编解码
/**
...
public extension ExpressibleByStringLiteral {
    ...
    func md5Encrypt(_ md5Type: MD5EncryptType = .lowercase32) -> String {
        guard (self.self as! String).count > 0 else {
            GConfig.log("⚠️⚠️⚠️md5加密无效的字符串⚠️⚠️⚠️")
            return ""
        }
        // 1.把待加密的字符串转成char类型数据 因为MD5加密是C语言加密
        let cCharArray = (self.self as! String).cString(using: .utf8)
        // 2.创建一个字符串数组接受MD5的值
    ...
    // MARK: 14.2、self64 编解码
    /// self64 编解码
    /// - Parameter encode: true:编码 false:解码
    /// - Returns: 编解码结果
    func self64String(encode: Bool) -> String? {
        guard encode else {
            // 1.解码
            guard let decryptionData = Data(base64Encoded: self.self as! String, options: .ignoreUnknownCharacters) else {
                return nil
            }
            return String(data: decryptionData, encoding: .utf8)
        }
        ...
        // 2.编码
        guard let codingData = (self.self as! String).data(using: .utf8) else {
            return nil
        }
        return codingData.base64EncodedString()
    }
...
/**
 iOS中填充规则PKCS7,加解密模式ECB(无补码,CCCrypt函数中对应的nil),字符集UTF8,输出self64(可以自己改hex)
 */
// MARK: 加密模式
...
public extension ExpressibleByStringLiteral {
    ...
    func shaCrypt(cryptType: DDYSHAType = .SHA1, key: String?, lower: Bool = true) -> String? {
        guard let cStr = (self.self as! String).cString(using: String.Encoding.utf8) else {
            return nil
        }
...
public extension String {
    ...
    func htmlEncodedStringToString() -> String? {
        ...
        guard let encodedData = self.self.data(using: String.Encoding.utf8), let attributedString = try? NSAttributedString(data: encodedData, options: attributedOptions, documentAttributes: nil) else {
            return nil
        }
