...
private let XYMenuWidth: Double = 140
...
class XYMenu: UIView {
    ...
    // MARK: 展示View菜单
    class func showMenuInView(images: [UIImage?]?,
                              titles: [String],
                              inView: UIView,
                              type: XYMenuType,
                              clickClosure: @escaping ItemClickBlock) {
    ...
    class func showMenuInBarButtonItem(images: [UIImage?]?,
                                       titles: [String],
                                       currentNavVC: UINavigationController,
                                       type: XYMenuType,
                                       clickClosure: @escaping ItemClickBlock) {
        let xy_menu = XYMenu.init()
    ...
    fileprivate func showMenuInView(images: [UIImage?]?,
                                    titles: [String],
                                    inView: UIView,
                                    type: XYMenuType,
                                    clickClosure: @escaping ItemClickBlock) {
    ...
    fileprivate func showMenuInBarButtonItem(images: [UIImage?]?,
                                             titles: [String],
                                             currentNavVC: UINavigationController,
                                             type: XYMenuType,
                                             clickClosure: @escaping ItemClickBlock) {
        if isShow {return}
        let statusHeight = Double(GConfig.TopSafeH)
        ...
        case .XYMenuRightNavBar:
            ...
            menuResultRect = CGRect(x: Double(kXYMenuScreenWidth) - 10 - XYMenuWidth,
                                    y: statusHeight + navigationBarHeight,
                                    width: XYMenuWidth,
                                    height: XYMenuHeight)
        default:
            break
    ...
    fileprivate func showAnimateMenu(images: [UIImage?]?, titles: [String], type: XYMenuType, closure: @escaping ItemClickBlock) {
        if isShow {return}
        isShow = true
...
extension UIBarButtonItem {
    ...
    public func xy_showXYMenu(images: [UIImage?]?,
                       titles: [String],
                       currentNavVC: UINavigationController,
                       type: XYMenuType,
                       closure: @escaping ItemClickBlock) {
        XYMenu.showMenuInBarButtonItem(images: images, titles: titles, currentNavVC: currentNavVC, type: type, clickClosure: closure)
    }
}

extension UIView {
    public func xy_showXYMenu(images: [UIImage?]?, titles: [String], type: XYMenuType, closure: @escaping ItemClickBlock) {
