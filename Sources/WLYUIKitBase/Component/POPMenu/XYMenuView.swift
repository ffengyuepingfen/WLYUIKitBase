...
private let kXYMenuContentBackColor = UIColor(white: 0.8, alpha: 1.0)
private let kXYMenuContentLineColor = UIColor(white: 0.5, alpha: 1.0)
private let kTriangleHeight: Double = 10
private let kTriangleLength: Double = 16.0

public typealias ItemClickBlock = (Int) -> Void

class XYMenuView: UIView {

    fileprivate var imagesArr: [UIImage?]?
    fileprivate var titlesArr: [String]?
    fileprivate var type: XYMenuType = .XYMenuMidNormal
    fileprivate var isDown: Bool = true
    fileprivate var itemClickBlock: ItemClickBlock?
    ...
    func setConfig(images: [UIImage?]!, titles: [String]!, rect: CGRect, menuType: XYMenuType, down: Bool, closure: ItemClickBlock!) {
    ...
    override func draw(_ rect: CGRect) {
        ...
        trianglePath.fill()
        if isDown {
            let radiusPath = UIBezierPath(roundedRect: CGRect(x: CGFloat(0.0),
                                                              y: CGFloat(kTriangleHeight),
                                                              width: bounds.size.width,
                                                              height: bounds.size.height - CGFloat(kTriangleHeight)),
                                          cornerRadius: 5.0)
            kXYMenuContentBackColor.set()
            radiusPath.fill()
        } else {
            let radiusPath = UIBezierPath(roundedRect: CGRect(x: CGFloat(0.0),
                                                              y: CGFloat(0.0),
                                                              width: bounds.size.width,
                                                              height: bounds.size.height - CGFloat(kTriangleHeight)), cornerRadius: 5.0)
            kXYMenuContentBackColor.set()
            radiusPath.fill()
        }
    }
    ...
    fileprivate func setMenuItems(rect: CGRect) {
        ...
        for i in 0 ..< Int(count) {
            ...
            if isDown {
                item.setUpViews(CGRect(x: 0.0,
                                       y: Double(i) * kContentItemHeight + kTriangleHeight,
                                       width: menuContentWidth,
                                       height: kContentItemHeight))
                itemBtn.frame = CGRect(x: 0.0,
                                       y: Double(i) * kContentItemHeight + kTriangleHeight,
                                       width: menuContentWidth,
                                       height: kContentItemHeight)
                if i != 0 {
                    ...
                    lineLayer.frame = CGRect(x: (kContentItemHeight / 3) - 4,
                                             y: Double(i) * kContentItemHeight + kTriangleHeight - 1,
                                             width: menuContentWidth - (kContentItemHeight) * 2.0 / 3.0 + 8,
                                             height: 0.5)
                ...
                if i != 0 {
                    ...
                    lineLayer.backgroundColor = kXYMenuContentLineColor.cgColor
                    lineLayer.frame = CGRect(x: (kContentItemHeight / 3) - 4,
                                             y: (Double(i) * kContentItemHeight) - 1,
                                             width: menuContentWidth - (kContentItemHeight) * 2.0 / 3.0 + 8,
                                             height: 0.5)
                    contentView.layer.addSublayer(lineLayer)
                }
