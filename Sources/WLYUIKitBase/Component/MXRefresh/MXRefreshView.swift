...
class MXRefreshView: UIView {
    ...
    init(superScrollView: UIScrollView, fillColor: UIColor = .systemBlue, duration: Double, operation : @escaping MXOperation) {
        
        let frame : CGRect = CGRect.init(x: 0, y: -300, width: superScrollView.frame.width, height: 300)
        
        super.init(frame: frame)
        //初始化动画
        self.initSubViews(color: fillColor)
        self.initAnimations()
        // 设置相关属性
    ...
    //MARK: Initialize
    private func initSubViews(color: UIColor){
        //WaveLayer
    ...
    private func initAnimations(){
        ...
        self.waveLayerAnimation.fillMode = CAMediaTimingFillMode.forwards
