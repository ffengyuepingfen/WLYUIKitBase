...
open class GYRollingNoticeView: UIView {
    ...
    open func reloadDataAndStartRoll() {
        ...
        timer = Timer.scheduledTimer(timeInterval: stayInterval,
                                     target: self,
                                     selector: #selector(GYRollingNoticeView.timerHandle),
                                     userInfo: nil,
                                     repeats: true)
...
//        print("count: \(count),  currentIndex:\(currentIndex)  willShowIndex: \(willShowIndex)")
        ...
        let currentCellIdx = self.reuseCells.firstIndex(of: currentCell!)
        let willShowCellIdx = self.reuseCells.firstIndex(of: willShowCell!)
